...
if __name__ == "__main__":
    ...
    root_path = os.path.join(os.getcwd(), "ClientsIncrease", "MNIST_IID_200")
    os.chdir(root_path)
    ...
    model_learning_rates = [10 ** (-i/2) for i in range(3, 7)]
    server_learning_rates = [10 ** (-i/2) for i in range(3, 7)]
    taus = [10 ** (-i) for i in range(4)]
    ...
    for tau in taus:
        for server_learning_rate in server_learning_rates:
            for model_learning_rate in model_learning_rates:
                ...
                simulation_configuration["orchestrator"]["optimizer"]["tau"] = tau

                simulation_configuration["orchestrator"]["archiver"]["orchestrator_filename"] = f"modellr_{model_learning_rate}__serverlr_{server_learning_rate}__tau_{tau}__central_testset.csv"
                simulation_configuration["orchestrator"]["archiver"]["central_on_local_filename"] = f"modellr_{model_learning_rate}__serverlr_{server_learning_rate}__tau_{tau}__local_testsets.csv"
